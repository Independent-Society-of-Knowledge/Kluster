...
import org.isk.kluster.base.message.CPU
import org.isk.kluster.base.message.Memory
import org.isk.kluster.base.message.Sensor
...
class ReportStatus {
    ...
    @Inject
    private lateinit var connector: SlaveConnector

    @Scheduled(cron = "{isk.kluster.monitor.cron}")
    fun reportSystemStatus() {
        with(systemInformationService) {
            connector.sendMessage(
                SystemStatusReportMessage(
                    CPU(
                        processorsLoad,
                        processorLoad,
                        processorMaxFreq,
                        processorsCurrentFreq,
                        processorCurrentFreq
                    ),
                    Sensor(
                        cpuTemperature,
                        cpuFanSpeed
