...
//    @Inject
//    private lateinit var slaveMessageHandlerService: MessageHandlerService

    @OnOpen
    fun open(session: Session) {
        logger.info("[{}] connected to master", session.stringFormat())
    }

    @OnMessage
    fun message(session: Session, msg: String) {
//        val message = objectMapper.readValue(msg, Message::class.java)
//        slaveMessageHandlerService.handleMessage(session, message)
    }
}