...
class MessageHandlerService {
    ...
    @Inject
    @All
    lateinit var handlers: MutableList<MessageHandler>


    fun handleMessage(s: Session, message: Message) {
        handlers
            .first { it.canHandle(message) }
            .also {
                logger.debug(
                    "[{}] handling message: {} with handle: {} ",
                    s.stringFormat(),
                    message.toString(),
